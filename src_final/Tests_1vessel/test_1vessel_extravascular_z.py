...
alpha=100
R=L[0]/alpha
cells_1D = np.array([40])


startVertex=np.array([0])
endVertex=np.array([1])
pos_vertex=np.array([[L_vessel/4,L_vessel/4,0.1],[L_vessel/4,L_vessel/4,L_vessel-0.1]])
vertex_to_edge=[[0],[0]]
...
plt.plot(net.pos_s[:,2],sol[-prob.S:], label='hybrid reaction')
...
corners=np.array([[0,0.25,0], [0,0.25,1],[0.5,0.25,0], [0.5,0.25,1]])*L_vessel
kk=GetPlaneReconstructionFast(240,1, 0, 2,  corners, res, prob)
plt.imshow(kk[0], extent=[0,L[0], 0, L[2]])
plt.xlabel("x")
plt.ylabel("z")
plt.colorbar()
plt.show()
...
corners=np.array([[0,0,0.5], [0,0.5,0.5],[0.5,0,0.5], [0.5,0.5,0.5]])*L_vessel
...
corners=np.array([[0.25,0,0], [0.25,0,1],[0.25,0.5,0], [0.25,0.5,1]])*L_vessel
