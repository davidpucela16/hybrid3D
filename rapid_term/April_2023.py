...
class Classic():
    ...
    def H_ii_non_singular(self,h):
        ...
        G = lambda theta :R*(1-np.cos(theta))*(4*R**2*np.sin(theta/2)**2)**-1.5
        return(integrate.quad(G,phi/2,np.pi*2-phi/2)[0]*R/(4*np.pi))
    ...
    def H_ii_numerical(self, cells):
        ...
        for theta in angle[1:]:
            ...
            integrand = np.append(integrand, 1/L)
    ...
    def get_single_layer_vessel(self, axial_disc):
        ...
        for i in range(len(x)):
            for j in range(len(x)):
                if i!=j:
                    inc_s=np.abs(x[i]-x[j])
                    G[i,j]=self.G_ij_elliptical(inc_s)
        
        G_self=self.G_ii_analytical(self.L/axial_disc)+0.28
        
        G[np.arange(len(x)),np.arange(len(x))]=G_self
        self.G=G
        return self.G*self.L/axial_disc
    ...
    def get_double_layer_vessel(self, axial_disc):
        ...
        H*=L/axial_disc
        H_self=self.H_ii_non_singular(self.L/axial_disc)*L/axial_disc+0.5
