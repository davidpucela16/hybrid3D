...
class cart_mesh_3D():
    ...
    def get_id(self,crds):
        """pos is an np.array with the values of the three coordinates
        The function returns the position along the mesh cells array"""
        pos_x=((crds[0])//(self.h/2))//2
        pos_y=((crds[1])//(self.h/2))//2
        pos_z=((crds[2])//(self.h/2))//2
        
        return(int(pos_x*len(self.z)*len(self.y)+pos_y*len(self.z)+pos_z))
    
    def get_8_closest(self,x):
        ...
        for i in x: #Loop through each of the axis 
            ...
            arr=np.append(arr, b)
        ID=self.get_id(x) #ID of the containing block
    ...
    def get_boundaries(self):
        ...
        for i in range(len(self.x)):
            east=np.concatenate((east, np.arange(self.step_y)+i*self.step_x))
            west=np.concatenate((west, np.arange(self.step_y)+i*self.step_x + self.corners[2]))
...
# =============================================================================
    ...
    def get_x_slice(self, crds):
        ...
        k=self.get_id(np.array([crds, 0,0])) #This is the lowest ID of the slice 
    ...
    def get_y_slice(self,crds):
        ...
        to the y axis closest to the coordinates given in crds
                ...
                crds is the coordinate along the relevant axis"""
        k=self.get_id(np.array([0,crds, 0])) #This is the lowest ID of the slice 
    ...
    def get_z_slice(self, crds):
        """ crds is the coordinate along the relevant axis"""
        k=self.get_id(np.array([0,0, crds])) #This is the lowest ID of the slice 
